...
class AdminProtect(Filter):
    async def __call__(self, message: Message):
        return message.from_user.id in [1526741555,]
...
async def newsletter_message(message: Message, state: FSMContext):
    ...
    for user in await get_users():
        try:
            ...
            pass
    await message.answer("Рассылка успешно завершина")
    await state.clear()
...
async def add_item_name(message: Message, state: FSMContext):
    ...
    await message.answer("Выбирете категорию товара", reply_markup=await kb.categories())
...
async def add_item_name(message: Message, state: FSMContext):
    ...
    await message.answer("Выбирете цену товара")
